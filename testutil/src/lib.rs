...
/// Metadata about the currently executing test, set by `#[testutil::test]`.
#[derive(Clone, Copy)]
...
thread_local! {
    static CURRENT_TEST: Cell<Option<CurrentTest>> = const { Cell::new(None) };
}

/// Set the current test context. Called by `#[testutil::test]`-generated code.
pub fn set_current_test(name: &'static str, module_path: &'static str) {
    CURRENT_TEST.set(Some(CurrentTest { name, module_path }));
}
...
/// A per-test resource. Generated by `#[testutil::fixture]` on a function.
///
/// Each test that requests the fixture via `#[fixture]` gets a fresh instance.
/// Cleanup: implement `Drop` on the struct.
pub trait Fixture: Send + Sync + Sized + 'static {
    fn setup() -> Result<Self, String>;
}

/// Requirement metadata for a fixture, generated by `#[testutil::fixture(...)]`.
pub trait FixtureMeta: Fixture {
...
pub struct TestDef {
    ...
    /// fixture type's `FixtureMeta::REQUIRES` slice.
